...
REPO="https://raw.githubusercontent.com/kingstorevpn/os/main/"
function CEKIP () {
MYIP=$(curl -sS ipv4.icanhazip.com)
IPVPS=$(curl -sS https://raw.githubusercontent.com/kingstorevpn/ip/main/ip | grep $MYIP | awk '{print $4}')
if [[ $MYIP == $IPVPS ]]; then
domain
Pasang
else
...
# ===== QUIET MODE & LOADER UI =====
# Sembunyikan output bawaan (wget/curl/apt) dan tampilkan loading yang rapi.
shopt -s expand_aliases

# Quiet aliases (tidak menampilkan link/progress)
alias wget='wget -q'
alias curl='curl -sS'
alias apt='DEBIAN_FRONTEND=noninteractive apt -y -qq'
alias apt-get='DEBIAN_FRONTEND=noninteractive apt-get -y -qq'

# ================= Spinner Themes ================
# Pilih tema via:   SPINNER_THEME=<tema> ./script.sh
# atau lewat argumen:  --spinner=<tema>
# Tema tersedia: dots | line | bounce | arc | moon | pulse
SPINNER_THEME="${SPINNER_THEME:-moon}"
_spinner_desc=""
_spinner_chars=""
...
apply_spinner_theme() {
  case "$1" in
    ...
    line)
      ...
      ;;
    bounce)
      _spinner_desc="Bounce (progress bertingkat)"
      _spinner_chars='▁▂▃▄▅▆▇▆▅▄▃▂'
      ;;
    arc)
      _spinner_desc="Arc (setengah lingkaran)"
      _spinner_chars='◜◠◝◞◡◟'
      ;;
    moon)
      _spinner_desc="Moon (fase bulan)"
      _spinner_chars='🌑🌒🌓🌔🌕🌖🌗🌘'
      ;;
    pulse)
      _spinner_desc="Pulse (denyut)"
      _spinner_chars='·∙●∙·'
      ;;
    *)
      _spinner_desc="Dots (klasik)"
...
# Parse argumen --spinner=<tema> bila ada
for _arg in "$@"; do
  case "$_arg" in
    --spinner=*)
      SPINNER_THEME="${_arg#*=}"
...
apply_spinner_theme "$SPINNER_THEME"
# =================================================

# Spinner keren untuk step yang panjang
run_with_spinner() {
  local _msg="$1"; shift
  local _log="/tmp/.step.$$.log"
  : > "$_log"
  tput civis 2>/dev/null || true
  # Jalankan perintah secara sunyi, log ke file (jaga-jaga untuk debug)
  ( "$@" >"$_log" 2>&1 ) &
  local _pid=$!
  local i=0
  printf "  [1;36m%s[0m [2m(%s)[0m " "$_msg" "$_spinner_desc"
  while kill -0 $_pid 2>/dev/null; do
    local _len=${#_spinner_chars}
    i=$(( (i+1) % _len ))
    # Ambil 1 char spinner
    local s="${_spinner_chars:i:1}"
    printf "
  [1;36m%s[0m [2m(%s)[0m [1;33m[%s][0m " "$_msg" "$_spinner_desc" "$s"
    sleep 0.12
  done
  wait $_pid
  local _status=$?
  if [ $_status -eq 0 ]; then
    printf "
  [1;36m%s[0m [2m(%s)[0m [1;32m[OK][0m        
" "$_msg" "$_spinner_desc"
  else
    printf "
  [1;36m%s[0m [2m(%s)[0m [1;31m[FAILED][0m    
" "$_msg" "$_spinner_desc"
    echo "---- LOG ----"
    tail -n 50 "$_log" || true
    echo "--------------"
...
# Wrapper untuk menjalankan fungsi step tanpa output terlihat
step() { # step "Judul" nama_fungsi
  local title="$1"; shift
  run_with_spinner "$title" "$@"
}
# ===== END QUIET MODE & LOADER UI =====
...
echo "KING STORE" > /etc/xray/username
echo ""
clear
author=$name
echo ""
...
cd
sleep 1
step "Mengonfigurasi Domain" res1
clear
rm /root/subdomainx
fi
}
...
sudo rm -f /etc/resolv.config >/dev/null 2>&1 || true
